...
def ppca(Y, d, dia):
    ...
    # Compute variance explained by each eigenvector
    eig_vals = np.sort(vals)[::-1][:d]
    total_var = np.sum(eig_vals)
    var_explained = eig_vals / total_var

    # Print variance explained for each eigenvector
    for i in range(d):
        print(f"Variance explained by eigenvector {i + 1}: {var_explained[i]}")

    return C, ss, M, X, Ye

    """
    In this modified code, after the computation of vecs (eigenvectors) and vals (eigenvalues), the code sorts the eigenvalues in descending order and selects the top d eigenvalues. Then, it computes the total variance explained by summing the selected eigenvalues and calculates the variance explained by each eigenvector as a ratio of its eigenvalue to the total variance.

The code then prints the variance explained for each eigenvector using a loop.

Please note that this code assumes you have the necessary dependencies (NumPy, SciPy) installed to run it.

# Selecting first k principal components
k = 2
reduced_data = Y @ C[:, :k]
